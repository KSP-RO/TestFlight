...
namespace TestFlightCore
{
    ...
    public class TestFlightEditorWindow : MonoBehaviourWindow
    {
        ...
        {
            base.OnGUIEvery();
            if (Event.current.type == EventType.Repaint && Visible && showGraph)
            {
                Vector2 startPoint = new Vector2(Screen.width - 200, Screen.height - 200);
                Vector2 endPoint = new Vector2(Screen.width - 100, Screen.height - 100);
                Color lineColor = XKCDColors.Yellow;
                Drawing.DrawLine(startPoint, endPoint, lineColor, 5f, true);
            }
        }

        internal override void DrawWindow(int windowID)
        {
