...
deploy:
	...
	python buildServer all --project-id 0 --project-name TestFlight --build-name $(TRAVIS_BRANCH)_$(TRAVIS_BUILD_NUMBER) --changelog "$(git log --format=%B --no-merges -n 1)" --files $(ZIP_CORE) $(ZIP_STOCK) $(ZIP_RO)
else
deploy:
